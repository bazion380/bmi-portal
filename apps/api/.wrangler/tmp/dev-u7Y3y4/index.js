...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/_internal/utils.mjs
// @__NO_SIDE_EFFECTS__
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/perf_hooks/performance.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/polyfill/performance.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/console.mjs
import { Writable } from "node:stream";

// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/mock/noop.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/node/console.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
globalThis.console = console_default;

// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/hrtime.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/process.mjs
import { EventEmitter } from "node:events";

// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/tty/read-stream.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/tty/write-stream.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/node-version.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/node/process.mjs
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
__export(schema_exports, {
  ...
  roleEnum: () => roleEnum,
  setupRLSAndTriggers: () => setupRLSAndTriggers,
  students: () => students,
  users: () => users
});
...
var applicationStatusEnum = pgEnum("application_status", ["pending", "accepted", "rejected", "waitlisted"]);
var enrollmentStatusEnum = pgEnum("enrollment_status", ["enrolled", "graduated", "withdrawn", "suspended"]);
var users = pgTable("users", {
  id: uuid("id").primaryKey(),
  // Maps to Neon Auth user.id
...
var financialHolds = pgTable("financial_holds", {
  id: serial("id").primaryKey(),
  studentId: integer("student_id").references(() => students.id).notNull(),
...
var verifyAuth = /* @__PURE__ */ __name(async (c, next) => {
...
// ../../packages/db/hr-schema.ts
var hr_schema_exports = {};
__export(hr_schema_exports, {
...
var staff = pgTable("staff", {
  ...
  // Maps loosely to the Neon Auth user ID from core-db
  department: text("department").notNull(),
...
var payrollRecords = pgTable("payroll_records", {
  id: serial("id").primaryKey(),
  staffId: serial("staff_id").references(() => staff.id).notNull(),
...
// src/routes/hr.ts
var hrRouter = new Hono2();
hrRouter.use("*", async (c, next) => {
  const sql2 = cs(c.env.HR_DATABASE_URL);
  c.set("hrDb", drizzle(sql2, { schema: hr_schema_exports }));
  await next();
});
...
hrRouter.patch("/leave-requests/:id", async (c) => {
  ...
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
  const [updated] = await db.update(leaveRequests).set({ status: body.status }).where(eq(leaveRequests.id, id)).returning();
...
var hr_default = hrRouter;

// ../../packages/db/library-schema.ts
var library_schema_exports = {};
__export(library_schema_exports, {
  books: () => books,
...
var borrowingRecords = pgTable("borrowing_records", {
  id: serial("id").primaryKey(),
  bookId: serial("book_id").references(() => books.id).notNull(),
  userId: uuid("user_id").notNull(),
  // Loosely references Neon Auth user ID from core-db (students/staff)
  borrowedAt: timestamp("borrowed_at").defaultNow().notNull(),
  dueDate: date("due_date").notNull(),
  returnedAt: timestamp("returned_at")
});
var libraryFines = pgTable("library_fines", {
  id: serial("id").primaryKey(),
...
var libraryRouter = new Hono2();
libraryRouter.use("*", async (c, next) => {
  const sql2 = cs(c.env.LIBRARY_DATABASE_URL);
  c.set("libraryDb", drizzle(sql2, { schema: library_schema_exports }));
  await next();
});
libraryRouter.get("/books", async (c) => {
  const db = c.get("libraryDb");
  const q = c.req.query("q");
  const books2 = q ? await db.select().from(books).where(like(books.title, `%${q}%`)) : await db.select().from(books);
  return c.json(books2);
});
...
libraryRouter.get("/borrowing", async (c) => {
  ...
  const records = await db.select({
    ...
    returnedAt: borrowingRecords.returnedAt,
    bookTitle: books.title,
    bookIsbn: books.isbn
  }).from(borrowingRecords).innerJoin(books, eq(borrowingRecords.bookId, books.id));
  return c.json(records);
});
libraryRouter.post("/borrowing", async (c) => {
  const db = c.get("libraryDb");
  const body = await c.req.json();
  const [record] = await db.insert(borrowingRecords).values(body).returning();
  return c.json(record, 201);
});
libraryRouter.patch("/borrowing/:id/return", async (c) => {
  const db = c.get("libraryDb");
  const id = parseInt(c.req.param("id"));
  const [updated] = await db.update(borrowingRecords).set({ returnedAt: /* @__PURE__ */ new Date() }).where(eq(borrowingRecords.id, id)).returning();
  return c.json(updated);
});
libraryRouter.get("/fines", async (c) => {
  const db = c.get("libraryDb");
  const fines = await db.select().from(libraryFines);
  return c.json(fines);
});
var library_default = libraryRouter;

// ../../packages/db/alumni-schema.ts
var alumni_schema_exports = {};
__export(alumni_schema_exports, {
  alumniProfiles: () => alumniProfiles,
  donations: () => donations,
  eventRegistrations: () => eventRegistrations,
  events: () => events
});
var alumniProfiles = pgTable("alumni_profiles", {
  id: serial("id").primaryKey(),
  userId: uuid("user_id").notNull().unique(),
  // Maps loosely to core-db user ID
...
var donations = pgTable("donations", {
  ...
  // e.g., 'Scholarship Fund', 'Campus Development'
  donatedAt: timestamp("donated_at").defaultNow().notNull()
});

// src/routes/alumni.ts
var alumniRouter = new Hono2();
alumniRouter.use("*", async (c, next) => {
  const sql2 = cs(c.env.ALUMNI_DATABASE_URL);
  c.set("alumniDb", drizzle(sql2, { schema: alumni_schema_exports }));
  await next();
});
alumniRouter.get("/profiles", async (c) => {
  const db = c.get("alumniDb");
  const profiles = await db.select().from(alumniProfiles);
  return c.json(profiles);
});
alumniRouter.post("/profiles", async (c) => {
  const db = c.get("alumniDb");
  const body = await c.req.json();
  const [created] = await db.insert(alumniProfiles).values(body).returning();
  return c.json(created, 201);
});
alumniRouter.get("/events", async (c) => {
  const db = c.get("alumniDb");
  const events2 = await db.select().from(events);
  return c.json(events2);
});
...
alumniRouter.post("/events/:id/register", async (c) => {
  ...
  const body = await c.req.json();
  const [reg] = await db.insert(eventRegistrations).values({ eventId, alumniId: body.alumniId }).returning();
  return c.json(reg, 201);
});
alumniRouter.get("/donations", async (c) => {
  const db = c.get("alumniDb");
  const donations2 = await db.select().from(donations);
  return c.json(donations2);
});
alumniRouter.post("/donations", async (c) => {
  const db = c.get("alumniDb");
  const body = await c.req.json();
  const [created] = await db.insert(donations).values(body).returning();
  return c.json(created, 201);
});
var alumni_default = alumniRouter;

// ../../packages/db/campus-services-schema.ts
var campus_services_schema_exports = {};
__export(campus_services_schema_exports, {
  hostelAllocations: () => hostelAllocations,
  hostels: () => hostels,
  transportPasses: () => transportPasses,
  transportRoutes: () => transportRoutes
});
var hostels = pgTable("hostels", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  capacity: integer("capacity").notNull(),
  location: text("location")
});
var hostelAllocations = pgTable("hostel_allocations", {
  id: serial("id").primaryKey(),
  hostelId: integer("hostel_id").references(() => hostels.id).notNull(),
  userId: uuid("user_id").notNull(),
  // Loosely references student/staff user ID
  roomNumber: text("room_number").notNull(),
  academicTerm: text("academic_term").notNull(),
  // e.g., 'Fall 2024'
  allocatedAt: timestamp("allocated_at").defaultNow().notNull()
});
var transportRoutes = pgTable("transport_routes", {
  id: serial("id").primaryKey(),
  routeName: text("route_name").notNull(),
  description: text("description"),
  vehicleNumber: text("vehicle_number")
});
var transportPasses = pgTable("transport_passes", {
  id: serial("id").primaryKey(),
...
// src/routes/campus-services.ts
var campusRouter = new Hono2();
campusRouter.use("*", async (c, next) => {
  const sql2 = cs(c.env.CAMPUS_DATABASE_URL);
  c.set("campusDb", drizzle(sql2, { schema: campus_services_schema_exports }));
  await next();
});
campusRouter.get("/hostels", async (c) => {
  const db = c.get("campusDb");
  const hostels2 = await db.select().from(hostels);
  return c.json(hostels2);
});
campusRouter.get("/hostels/:id/allocations", async (c) => {
  const db = c.get("campusDb");
  const hostelId = parseInt(c.req.param("id"));
  const allocations = await db.select().from(hostelAllocations).where(eq(hostelAllocations.hostelId, hostelId));
  return c.json(allocations);
});
campusRouter.post("/hostels/allocate", async (c) => {
  const db = c.get("campusDb");
  const body = await c.req.json();
  const [alloc] = await db.insert(hostelAllocations).values(body).returning();
...
campusRouter.get("/transport/passes", async (c) => {
  ...
  const passes = await db.select({
    ...
    validUntil: transportPasses.validUntil,
    issuedAt: transportPasses.issuedAt,
    routeName: transportRoutes.routeName,
    vehicleNumber: transportRoutes.vehicleNumber
  }).from(transportPasses).innerJoin(transportRoutes, eq(transportPasses.routeId, transportRoutes.id));
  return c.json(passes);
});
campusRouter.post("/transport/passes", async (c) => {
  const db = c.get("campusDb");
  const body = await c.req.json();
  const [pass] = await db.insert(transportPasses).values(body).returning();
  return c.json(pass, 201);
});
var campus_services_default = campusRouter;
...
app.get(
  "/health",
  (c) => c.json({ status: "ok", service: "BMI UMS API", timestamp: (/* @__PURE__ */ new Date()).toISOString() })
);
var api = new Hono2();
api.use("*", verifyAuth);
...
api.get("/academics/offerings", async (c) => {
  ...
  }).from(courseOfferings).innerJoin(courses, eq(courseOfferings.courseId, courses.id));
...
api.post("/academics/register", async (c) => {
  ...
  try {
    const [registration] = await db.insert(registrations).values({ studentId: student.id, courseOfferingId: body.courseOfferingId }).returning();
...
api.get("/academics/grades", async (c) => {
  ...
  const gradesData = await db.select({
    ...
    term: courseOfferings.term
  }).from(grades).innerJoin(registrations, eq(grades.registrationId, registrations.id)).innerJoin(courseOfferings, eq(registrations.courseOfferingId, courseOfferings.id)).innerJoin(courses, eq(courseOfferings.courseId, courses.id)).where(eq(registrations.studentId, student.id));
...
api.get("/finance/holds", async (c) => {
  ...
  const holds = await db.select().from(financialHolds).where(
    and(
      eq(financialHolds.studentId, student.id),
      eq(financialHolds.isActive, true)
    )
  );
  return c.json(holds);
});
...
api.route("/hr", hr_default);
api.route("/library", library_default);
api.route("/alumni", alumni_default);
api.route("/campus", campus_services_default);
app.route("/api/v1", api);
var src_default = app;

// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-8CFd5G/middleware-insertion-facade.js
...
// C:/Users/nissi/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-8CFd5G/middleware-loader.entry.ts
